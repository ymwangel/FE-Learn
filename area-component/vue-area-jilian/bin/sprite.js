...
// var final = {
//   yourcss:'../css/search.css',
//   yoursprite:'../i/sprite.png',
//   yourcsssprite:'/m-search/pages/good/i/sprite.png',
//   topx:function(a){ return a + 'px'} // 如果是2倍雪碧图 return (a/2) + 'px'
// }
var final = {
  yourcss:'../css/metal.css',
  yoursprite:'../i/metal.png',
  yourcsssprite:'/inner/pages/metal-factory/i/metal.png',
  topx:function(a){ return (a/2) + 'px'} // 如果是2倍雪碧图 return (a/2) + 'px'
}
...
function setSprite(path,imginfo,rule){
  ...
  if(backgroundSize){
    backgroundSize.value = final.topx(imginfo.width)
  }else{
    rule.declarations.push({
      type:'declaration',
      property:'background-size',
      value:final.topx(imginfo.width)
    })
  }
...
function setIcon(icon,info){
  ...
  if(width){
    width.value = final.topx(info.width)
  }else{
    icon.declarations.push({
      type:'declaration',
      property:'width',
      value:final.topx(info.width)
    })
  }
  var height = _.find(_.propEq('property','height'),icon.declarations)
  if(height){
    height.value = final.topx(info.height)
  }else{
    icon.declarations.push({
      type:'declaration',
      property:'height',
      value:final.topx(info.height)
    })
  }
  var position = _.find(_.propEq('property','background-position'),icon.declarations)
  if(position){
    position.value = final.topx(0-info.x) + ' ' + final.topx(0-info.y)
  }else{
    icon.declarations.push({
      type:'declaration',
      property:'background-position',
      value:final.topx(0-info.x) + ' ' + final.topx(0-info.y)
    })
  }
}
function handlerCss(cssTokens,fn){
  ...
  spritesmith.run({
    ...
    if(err){
      ...
      fs.writeFileSync(final.yoursprite,result.image)
      setSprite(final.yourcsssprite,result.properties,dicon)
...
function main(){
  var cssstring = fs.readFileSync(final.yourcss).toString()
  ...
  if(sheet.parsingErrors.length == 0){
    handlerCss(sheet.rules,function(){
      ...
      fs.writeFileSync(final.yourcss,cssstr)
    })
